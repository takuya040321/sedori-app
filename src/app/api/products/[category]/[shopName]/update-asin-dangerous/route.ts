// src/app/api/products/[category]/[shopName]/update-asin-dangerous/route.ts
...
export async function POST(
  ...
  const { productIndex, asinIndex, isDangerousGoods } = await request.json();
  ...
  try {
    ...
    // 危険物フラグを更新
    shopData.products[productIndex].asins[asinIndex].isDangerousGoods = isDangerousGoods;

    // ファイルに書き戻す
    await fs.writeFile(filePath, JSON.stringify(shopData, null, 2), "utf-8");

    return NextResponse.json({ success: true, message: "危険物フラグを更新しました" });
  } catch (error) {
