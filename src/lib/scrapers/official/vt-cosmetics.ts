import { Product, ShopData } from '@/types/product'
import { sleep } from '@/lib/utils'

// Mock scraper for VT Cosmetics
export async function scrapeVTCosmetics(): Promise<ShopData> {
  // Simulate scraping delay
  await sleep(2000 + Math.random() * 3000)
  
  const mockProducts: Product[] = [
    {
      productName: "VT CICA マイルド フォーム クレンザー 300ml",
      imageUrl: "https://images.unsplash.com/photo-1556228578-8c89e6adf883?w=300&h=300&fit=crop",
      price: 1980,
      salePrice: 1584,
      asin: "VT001",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA スリーピング マスク 30ml",
      imageUrl: "https://images.unsplash.com/photo-1571781926291-c477ebfd024b?w=300&h=300&fit=crop",
      price: 2200,
      asin: "VT002",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA カプセル マスク 7.5ml×10個",
      imageUrl: "https://images.unsplash.com/photo-1596755389378-c31d21fd1273?w=300&h=300&fit=crop",
      price: 3300,
      salePrice: 2640,
      asin: "VT003",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA バイタル トナー 200ml",
      imageUrl: "https://images.unsplash.com/photo-1620916566398-39f1143ab7be?w=300&h=300&fit=crop",
      price: 2750,
      asin: "VT004",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA ハイドレーティング エッセンス 50ml",
      imageUrl: "https://images.unsplash.com/photo-1611930022073-b7a4ba5fcccd?w=300&h=300&fit=crop",
      price: 3080,
      salePrice: 2464,
      asin: "VT005",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA サン スティック SPF50+ PA++++ 18g",
      imageUrl: "https://images.unsplash.com/photo-1556228453-efd6c1ff04f6?w=300&h=300&fit=crop",
      price: 1650,
      asin: "VT006",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA リップ バーム 3.5g",
      imageUrl: "https://images.unsplash.com/photo-1586495777744-4413f21062fa?w=300&h=300&fit=crop",
      price: 990,
      salePrice: 792,
      asin: "VT007",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA アイ クリーム 30ml",
      imageUrl: "https://images.unsplash.com/photo-1598300042247-d088f8ab3a91?w=300&h=300&fit=crop",
      price: 2860,
      asin: "VT008",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA セラム 30ml",
      imageUrl: "https://images.unsplash.com/photo-1570194065650-d99fb4bedf0a?w=300&h=300&fit=crop",
      price: 3520,
      salePrice: 2816,
      asin: "VT009",
      updatedAt: new Date().toISOString()
    },
    {
      productName: "VT CICA モイスチャライザー 75ml",
      imageUrl: "https://images.unsplash.com/photo-1612817288484-6f916006741a?w=300&h=300&fit=crop",
      price: 2420,
      asin: "VT010",
      updatedAt: new Date().toISOString()
    }
  ]
  
  return {
    lastUpdated: new Date().toISOString(),
    products: mockProducts
  }
}